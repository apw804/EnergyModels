...
# Scenario: Runs Mona's suggestion of randomly turning off 1-2 cells between timesteps.
# Different implementation of the scenario that removes the cell from the 
...
import numpy as np
from AIMM_simulator import Sim, Logger, np_array_to_str, to_dB, NR_5G_standard_functions, Scenario
...
class CellOff(Scenario):
    ...
    def delete_cell(self, cell_id):
        if cell_id in self.sim.cells:
            self.sim.cells.remove(cell_id)

    def loop(self):
        while True:
            if self.delay_time is not None:
                if self.kill_cells is not None:
                    if self.sim.env.now >= self.delay_time:
                        for i in self.kill_cells:
                            self.delete_cell(i)
