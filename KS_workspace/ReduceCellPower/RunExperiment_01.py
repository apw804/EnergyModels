...
# Define the seeds range
seeds_list = list(range(10))

# Define power_dBm range
power_dBm_list = list((range(30, 50)))
power_dBm_list_floats = [float(q) for q in power_dBm_list]

if __name__ == '__main__':
    with IncrementalBar('Processing seeds:', max=len(seeds_list)) as seed_bar:
        for i in seeds_list:
            with Bar('Processing power_dBm:', max=len(power_dBm_list_floats)) as power_bar:
                for j in power_dBm_list_floats:
