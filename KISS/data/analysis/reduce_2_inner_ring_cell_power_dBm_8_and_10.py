...
data_path = project_path / 'data' / 'output' / 'reduce_cell_8_and_10_power' / '2023_04_26'
rcp_data = data_path
cells_turned_down = []
...
# Group by reduced_cell_8_power (and cell_10) for the whole network
...
# Group by reduced_cell_8_power (also cell_10) AND serving_cell_id for the inner ring cells
df_inner_ring_stats = df_inner_ring_cells.groupby(['reduced_cell_8_power', 'serving_cell_id']).agg({
    'cell_throughput(Mb/s)': ['mean', 'std'],
    'cell_power(kW)': ['mean', 'std'],
    'cell_ee(bits/J)': ['mean', 'std'],
    'cell_se(bits/Hz)': ['mean', 'std']
...
# Group by reduced_cell_8_power (also cell_10) AND serving_cell_id for the other cells
df_other_cells_stats = df_other_cells.groupby(['reduced_cell_8_power', 'serving_cell_id']).agg({
    'cell_throughput(Mb/s)': ['mean', 'std'],
    'cell_power(kW)': ['mean', 'std'],
    'cell_ee(bits/J)': ['mean', 'std'],
    'cell_se(bits/Hz)': ['mean', 'std']
...
# Get the mean and standard deviaton of the inner ring cells and other cells per reduced_cell_8_power
df_reduced_power_cells = df_inner_ring_stats.groupby(['reduced_cell_8_power']).agg({
...
# Get the mean and standard deviaton of the inner ring cells and other cells per reduced_cell_8_power
df_full_power_cells = df_other_cells_stats.groupby(['reduced_cell_8_power']).agg({
    'cell_throughput(Mb/s)_mean': ['mean', 'std'],
    'cell_power(kW)_mean': ['mean', 'std'],
    'cell_ee(bits/J)_mean': ['mean', 'std'],
    'cell_se(bits/Hz)_mean': ['mean', 'std']
...
# Set figure title
fig.suptitle(f'Effects of reducing the output power (dBm) of cell 8 and cell 10 on the network', fontsize=16)

# Convert `reduced_cell_8_power` from dBm to watts
df_reduced_power_cells['reduced_cell_8_power_watts'] = 10**(df_reduced_power_cells['reduced_cell_8_power']/10)/1000
df_full_power_cells['reduced_cell_8_power_watts'] = 10**(df_full_power_cells['reduced_cell_8_power']/10)/1000
...
ax[0,0].errorbar(df_reduced_power_cells['reduced_cell_8_power'],
...
ax[0,0].errorbar(df_full_power_cells['reduced_cell_8_power'],
...
ax[0,0].legend()
ax[0,0].grid(True)
ax[0,0].set_title('Cell Throughput vs. Reduced Tx Cell (8 & 10) Power')


# Drop the value at `reduce_cell_8_power` of 0
df_reduced_power_cells = df_reduced_power_cells[df_reduced_power_cells['reduced_cell_8_power'] != 0]
df_full_power_cells = df_full_power_cells[df_full_power_cells['reduced_cell_8_power'] != 0]



# Cell power(kW)
# `reduced_cell_8_power_watts` with -999 value is not plotted
ax[0,1].errorbar(df_reduced_power_cells[df_reduced_power_cells['reduced_cell_8_power'] != -999]['reduced_cell_8_power'],
            df_reduced_power_cells[df_reduced_power_cells['reduced_cell_8_power'] != -999]['cell_power(kW)_mean_mean'],
            yerr=df_reduced_power_cells[df_reduced_power_cells['reduced_cell_8_power'] != -999]['cell_power(kW)_mean_std'],
            fmt='o',
            capsize=2,
            linewidth=1,
...
sleep_power_watts = df_reduced_power_cells[df_reduced_power_cells['reduced_cell_8_power'] == -999]['reduced_cell_8_power']
ax[0,1].plot(0,
            df_reduced_power_cells[df_reduced_power_cells['reduced_cell_8_power'] == -999]['cell_power(kW)_mean_mean'],
            marker='o',
            markersize=5,
...
ax[0,1].errorbar(df_full_power_cells['reduced_cell_8_power'],
...
ax[0,1].legend()
ax[0,1].grid(True)
ax[0,1].set_title('Cell Power vs. Reduced Tx Cell (8 & 10) Power')


# CELL EE (bits/J)
ax[1,0].errorbar(df_reduced_power_cells['reduced_cell_8_power'],
...
ax[1,0].errorbar(df_full_power_cells['reduced_cell_8_power'],
...
ax[1,0].legend()
ax[1,0].grid(True)
ax[1,0].set_title('Cell EE vs. Reduced Tx Cell (8 & 10) Power')


# CELL SE (bits/Hz)
ax[1,1].errorbar(df_reduced_power_cells['reduced_cell_8_power'],
...
ax[1,1].errorbar(df_full_power_cells['reduced_cell_8_power'],
...
ax[1,1].legend()
ax[1,1].grid(True)
ax[1,1].set_title('Cell SE vs. Reduced Tx Cell (8 & 10) Power')
...
# Save the figure
fig.savefig('2023_05_03T_r2irp_cell_vs_reduced_tx_cell_8_and_10_power_dBm.pdf', dpi=300, format='pdf', bbox_inches='tight')

print(df_reduced_power_cells['reduced_cell_8_power'])
