...
data_path = project_path / 'data' / 'output' / 'reduce_all_inner_ring_cell_power' / '2023_05_04'
rcp_data = data_path
cells_turned_down = []
...
# Write df_concat to a feather file
# df_concat.to_feather(data_path / 'df_concat.feather')


def get_variable_power_cells(df):
    """Get a list of cell IDs that have variable power levels."""
    ...
    # Set the index to the serving_cell_id
    df_temp.set_index('serving_cell_id', inplace=True)

    # For each of the cells in the index, get the unique values in the sc_power(dBm) column
    for cell in df_temp.index.unique():
        # Get the unique values in the sc_power(dBm) column
        unique_values = df_temp.loc[cell, 'sc_power(dBm)'].unique()

        # If there are more than 1 unique values, then return the cell
        if len(unique_values) > 1:
            variable_power_cells.append(cell)

    return variable_power_cells

print(get_variable_power_cells(df_concat))



def get_constant_power_cells(df):
    """Get a list of cell IDs that have constant power levels"""

    constant_power_cells = []
    ...
    for cell in df_temp.index.unique():
        ...
        # If there is only 1 unique value, then return the cell
        if len(unique_values) == 1:
            constant_power_cells.append(cell)

    return constant_power_cells

print(get_constant_power_cells(df_concat))
...
for cell in cells_turned_down:
    ...
    for seed in unique_seeds:
        ...
        df_concat = df_concat.append({
            ...
            # For the reduced_cell_x_power columns, where x is NOT the cell that is turned down, set the value to 0.0
...
def calc_mean_energy_efficiency(df):
    """
    Calculate the energy efficiency of the network
    """
    # Get the total throughput of the network
    total_throughput = df['cell_throughput(Mb/s)'].sum()

    # Get the total power of the network
    total_power = df['cell_power(kW)'].sum()
...
def calc_mean_spectral_efficiency(df):
    """
    Calculate the spectral efficiency of the network
    """
    # Get the total throughput of the network
    total_throughput = df['cell_throughput(Mb/s)'].sum()

    # Get the total bandwidth of the network
    total_bandwidth = df['bandwidth(MHz)'].sum()

    # Number of cells in the network
    num_cells = len(df)

    # Calculate the spectral efficiency
    spectral_efficiency = total_throughput / (total_bandwidth * num_cells)

    return spectral_efficiency

def calc_mean_throughput(df):
    """
    Calculate the mean throughput of the network
    """
    # Get the total throughput of the network
    total_throughput = df['cell_throughput(Mb/s)'].sum()

    # Get the number of cells in the network
    num_cells = len(df)

    # Calculate the mean throughput
    mean_throughput = total_throughput / num_cells

    return mean_throughput

def calc_mean_power_cons(df):
    """
    Calculate the mean power of the network
    """
    # Get the total power of the network
    total_power = df['cell_power(kW)'].sum()
    ...
    mean_power = total_power / num_cells

    return mean_power

def get_df_sets(df, rcp_cells):
    """ Produces dataframes for the different sets of cells in the network"""

    # Get all the cells in the network
    df_all_cells = df.loc[df['serving_cell_id'].unique()]

    # Get the cells in the inner ring
    inner_ring_cells = [4,5,8,10,13,14]
    df_inner_ring_cells = df.loc[df['serving_cell_id'].isin(inner_ring_cells)]

    # Get the cells in the outer ring
    outer_ring_cells = [1,2,3,6,7,11,12,15,16,17,18]
    df_outer_ring_cells = df.loc[df['serving_cell_id'].isin(outer_ring_cells)]

    # Get the cells with constant power
    df_constant_power_cells = df.loc[~df['serving_cell_id'].isin(rcp_cells)]

    # Get the cells with variable power
    df_variable_power_cells = df.loc[df['serving_cell_id'].isin(rcp_cells)]
    
    return df_all_cells, df_inner_ring_cells, df_outer_ring_cells, df_constant_power_cells, df_variable_power_cells


df_all_cells, df_inner_ring_cells, df_outer_ring_cells, df_constant_power_cells, df_variable_power_cells = get_df_sets(df_concat, rcp_cells)
...
# Group by reduced_cell_4_power (and rest of inner ring) for the whole network
...
# Group by reduced_cell_4_power (and rest of inner ring) AND serving_cell_id for the inner ring cells
df_inner_ring_stats = df_inner_ring_cells.groupby(['reduced_cell_4_power', 'serving_cell_id']).agg({
    'cell_throughput(Mb/s)': ['mean', 'std'],
    'cell_power(kW)': ['mean', 'std'],
    'cell_ee(bits/J)': ['mean', 'std'],
    'cell_se(bits/Hz)': ['mean', 'std']
...
# Group by reduced_cell_4_power (and rest of inner ring) AND serving_cell_id for the other cells
df_other_cells_stats = df_other_cells.groupby(['reduced_cell_4_power', 'serving_cell_id']).agg({
    'cell_throughput(Mb/s)': ['mean', 'std'],
    'cell_power(kW)': ['mean', 'std'],
    'cell_ee(bits/J)': ['mean', 'std'],
    'cell_se(bits/Hz)': ['mean', 'std']
...
# Get the mean and standard deviaton of the inner ring cells and other cells per reduced_cell_4_power
df_reduced_power_cells = df_inner_ring_stats.groupby(['reduced_cell_4_power']).agg({
...
# Get the mean and standard deviaton of the inner ring cells and other cells per reduced_cell_4_power
df_full_power_cells = df_other_cells_stats.groupby(['reduced_cell_4_power']).agg({
    'cell_throughput(Mb/s)_mean': ['mean', 'std'],
    'cell_power(kW)_mean': ['mean', 'std'],
    'cell_ee(bits/J)_mean': ['mean', 'std'],
    'cell_se(bits/Hz)_mean': ['mean', 'std']
...
# Set figure title
fig.suptitle(f'Effects of reducing the output power (dBm) of inner ring cells (4,5,8,10,13,14) on the network', fontsize=16)

# Convert `reduced_cell_4_power` from dBm to watts
df_reduced_power_cells['reduced_cell_4_power_watts'] = 10**(df_reduced_power_cells['reduced_cell_4_power']/10)/1000
df_full_power_cells['reduced_cell_4_power_watts'] = 10**(df_full_power_cells['reduced_cell_4_power']/10)/1000
...
ax[0,0].errorbar(df_reduced_power_cells['reduced_cell_4_power'],
            df_reduced_power_cells['cell_throughput(Mb/s)_mean_mean'],
            yerr=df_reduced_power_cells['cell_throughput(Mb/s)_mean_std'],
            fmt='.',
            capsize=2,
            linewidth=1,
            label='Reduced Tx Power Cells Mean (Inner Ring)',
)

ax[0,0].errorbar(df_full_power_cells['reduced_cell_4_power'],
...
ax[0,0].set_xlabel('Reduced Cell Tx Power (dBm)')
ax[0,0].set_ylabel('Mean Cell Throughput (Mb/s)')
ax[0,0].legend()
ax[0,0].grid(True)
ax[0,0].set_title('Cell Throughput vs Reduced Tx Cell (4,5,8,10,13,14) Power')


# Drop the value at `reduce_cell_8_power` of 0
df_reduced_power_cells = df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] != 0]
df_full_power_cells = df_full_power_cells[df_full_power_cells['reduced_cell_4_power'] != 0]



# Cell power(kW)
# `reduced_cell_4_power_watts` with -999 value is not plotted
ax[0,1].errorbar(df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] != -999]['reduced_cell_4_power'],
            df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] != -999]['cell_power(kW)_mean_mean'],
            yerr=df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] != -999]['cell_power(kW)_mean_std'],
            fmt='o',
            capsize=2,
            linewidth=1,
...
sleep_power_watts = df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] == -999]['reduced_cell_4_power_watts']
ax[0,1].plot(0,
            df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] == -999]['cell_power(kW)_mean_mean'],
            marker='o',
            markersize=5,
...
ax[0,1].errorbar(df_full_power_cells['reduced_cell_4_power'],
...
# Set the x and y axis labels
ax[0,1].set_xlabel('Reduced Cell Tx Power (dBm)')
ax[0,1].set_ylabel('Mean Cell Power (kW)')
ax[0,1].legend()
ax[0,1].grid(True)
ax[0,1].set_title('Cell Power vs Reduced Tx Cell (4,5,8,10,13,14) Power')



ax[1,0].errorbar(df_reduced_power_cells['reduced_cell_4_power'],
...
ax[1,0].errorbar(df_full_power_cells['reduced_cell_4_power'],
...
# Set the x and y axis labels
ax[1,0].set_xlabel('Reduced Cell Tx Power (dBm)')
ax[1,0].set_ylabel('Mean Cell EE (bits/J)')
ax[1,0].legend()
ax[1,0].grid(True)
ax[1,0].set_title('Cell EE vs Reduced Tx Cell (4,5,8,10,13,14) Power')



ax[1,1].errorbar(df_reduced_power_cells['reduced_cell_4_power'],
...
ax[1,1].errorbar(df_full_power_cells['reduced_cell_4_power'],
...
# Set the x and y axis labels
ax[1,1].set_xlabel('Reduced Cell Tx Power (dBm)')
ax[1,1].set_ylabel('Mean Cell SE (bits/Hz)')
ax[1,1].legend()
ax[1,1].grid(True)
ax[1,1].set_title('Cell SE vs Reduced Tx Cell (4,5,8,10,13,14) Power')
...
# Save the figure
fig.savefig('2023_05_04T_raircp_cell_vs_reduced_tx_all_inner_ring_cell_power_dBm.pdf', dpi=300, format='pdf', bbox_inches='tight')
