# Script to plot results of the reduce_2_inner_ring_cell_power experiment

# CONTEXT
# -------
# Two cells that are opposite each other in the inner ring are turned down to 0W and the rest of 
# the cells stay at 43 dBm.
#
# We'll be modelling cell sleep by assuming that the cells that are turned down to 0W are OFF and
# adding a predefined power consumption to these cells to represent the power consumption of the 
# cell when it is in sleep mode.
# 
...
data_path = project_path / 'data' / 'output' / 'reduce_cell_4_and_14_power' / '2023_04_26'
rcp_data = data_path
cells_turned_down = []
...
# Group by reduced_cell_4_power (and cell_14) for the whole network
...
# Group by reduced_cell_4_power (also cell_14) AND serving_cell_id for the inner ring cells
...
# Group by reduced_cell_4_power (also cell_14) AND serving_cell_id for the other cells
...
# Set figure title
fig.suptitle(f'Effects of reducing the output power (dBm) of cell 4 and cell 14 on the network', fontsize=16)
...
ax[0,0].legend()
ax[0,0].grid(True)
ax[0,0].set_title('Cell Throughput vs. Reduced Tx Cell (4 & 14) Power')


# Drop the value at `reduced_cell_4_power` of 0
...
sleep_power_watts = df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] == -999]['reduced_cell_4_power']
ax[0,1].plot(0,
            df_reduced_power_cells[df_reduced_power_cells['reduced_cell_4_power'] == -999]['cell_power(kW)_mean_mean'],
            marker='o',
            markersize=5,
...
ax[0,1].legend()
ax[0,1].grid(True)
ax[0,1].set_title('Cell Power vs. Reduced Tx Cell (4 & 14) Power')
...
ax[1,0].legend()
ax[1,0].grid(True)
ax[1,0].set_title('Cell EE vs. Reduced Tx Cell (4 & 14) Power')
...
ax[1,1].legend()
ax[1,1].grid(True)
ax[1,1].set_title('Cell SE vs. Reduced Tx Cell (4 & 14) Power')
...
# Save the figure
fig.savefig('2023_05_03T_r2irp_cell_vs_reduced_tx_cell_4_and_14_power_dBm.pdf', dpi=300, format='pdf', bbox_inches='tight')

print(df_reduced_power_cells['reduced_cell_4_power'])
...
# Plot the cell_throughput(Mb/s) mean and standard deviation for each serving_cell_id


# Plot the cell_power(kW) mean and standard deviation 
