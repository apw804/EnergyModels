...
# "For a hypothetical user with a single resource element they see the same SINR as a hypothetical user with all the resource elements but they receive only a fraction of the throughput.""
# We could simulate this at the PRB level, by saying UE[1] gets ONE of the PRBs available (1/24) and UE[2] gets ALL OTHER PRBs available (23/24)
...
def main():
  ...
  ue1 = sim.UEs[0]
  ue1.radio_state = NR_5G_standard_functions.Radio_state
  ue1.radio_state.nPRB = 1

  # Create UE[2] at Cell Edge (500.,0.,1.5)opposite side of the cell
  ue2_xyz = [500.,0.,1.5]
  sim.make_UE(xyz=ue2_xyz, pathloss_model=pl_uma_nlos).attach_to_strongest_cell_simple_pathloss_model() 
  ue2 = sim.UEs[1]
  ue2_tp = ue2.send_subband_cqi_report()

  # Plot the UEs on the axes
  plt.scatter(ue1_xyz[0],ue1_xyz[1])
  plt.scatter(ue2_xyz[0],ue2_xyz[1])
  ...
  print(f'\n')
  print(f'UE[2] distance to cell (m): {np.linalg.norm(cell0.xyz[:1] - ue2.xyz[:1]):,.2f}')
  print(f'UE[2] UMa NLOS pathloss (dB): {pl_uma_nlos.__call__(xyz_cell=cell0.xyz, xyz_UE=ue2.xyz)}')
  print(f'UE[2] SINR (dB): {ue2.sinr_dB[0]}')
