...
# v10.1 FIXME: issue where energy logger only records from time=1
...
class SimLogger(Logger):
    ...
    def __init__(s, sim, seed, until, energy_model=None, sim_args=None, func=None, header='', f=stdout,
...
class CellLogger(Logger):
    ...
    def __init__(s, sim, energy_model, until, sim_args=None, func=None, header='', f=stdout,
                 logging_interval=1.0):
...
class UeLogger(Logger):
    ...
    def get_ue_data(s):
        ...
        data = {
            ...
            "ue_neighbouring_cell_interference_dBm": "TBC (AIMM v2.0.2)",   # Use the SINR to reverse engineer the interference
...
class EnergyLogger(Logger):
    ...
    def __init__(s, sim, seed, energy_model, until, func=None, header='', f=stdout,
                 logging_interval=1.0):
    ...
    def finalize(s):
        SimLogger.write_log(logtype_="Energy", df=s.main_dataframe)
...
def test_01(seed=0, subbands=1, isd=5000.0, sim_radius=2500.0, nues=1, until=100.0, author='Kishan Sthankiya',
    ...
    for i in ue_ppp:
        x, y = i
        ue_xyz = x, y, 1.5
        sim.make_UE(xyz=ue_xyz, reporting_interval=1e-2).attach_to_strongest_cell_simple_pathloss_model()
    em = Energy(sim)
    ...
    sim_logger = SimLogger(sim=sim, seed=seed, until=until, sim_args=sim_args_dict, logging_interval=1.0)
    energy_logger = EnergyLogger(sim=sim, seed=seed, energy_model=em, until=until)
    cell_logger = CellLogger(sim=sim, energy_model=em, until=until)
    ue_logger = UeLogger(sim=sim, energy_model=em,until=until)
