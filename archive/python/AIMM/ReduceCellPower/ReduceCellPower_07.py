...
class QmCellLogger(Logger):
    ...
    def __init__(s, sim, seed, energy_model, until, sim_args=None, func=None, header='', f=stdout,
        ...
        super(QmCellLogger, s).__init__(sim, func, header, f, logging_interval, np_array_to_str=np_array_to_str)
        s.sim_args = sim_args
    ...
    def log_folder():
        ...
        today_folder = Path(logging_path + '/' + QmCellLogger.date_str())
        today_folder.mkdir(parents=True, exist_ok=True)
        return str(today_folder)

    _LOGTYPES = Literal["Sim", "Args", "Profile"]

    def get_logfile_name(s, logtype_: _LOGTYPES = None):
        if logtype_ is not None:
            class_name = s.__class__.__name__
            options = get_args(s._LOGTYPES)
            assert logtype_ in options, f"'{logtype_}' is not in {options}"
            return str(Path(__file__).stem) + '_' + logtype_ + 'Log_' + class_name + '_' + QmCellLogger.time_str()
        raise Exception(f"A logfile type MUST be specified. E.g. {get_args(QmCellLogger._LOGTYPES)}")

    def write_sim_log(s):
        filepath = Path(QmCellLogger.log_folder() + '/' + s.get_logfile_name(logtype_="Sim"))
        s.cell_dataframe.to_csv(str(filepath) + '.tsv', index=False, sep='\t', na_rep='NaN', header=True,
                                float_format='%g')

    def write_args_log(s, sim_args_dict):
        filepath = Path(QmCellLogger.log_folder() + '/' + s.get_logfile_name(logtype_="Args") + ".json")
...
class QmEnergyLogger(Logger):
    ...
    def write_sim_log(s):
        filepath = Path(QmEnergyLogger.log_folder() + '/' + s.get_logfile_name(logtype_="Sim"))
        s.main_dataframe.to_csv(str(filepath) + '.tsv', index=False, sep='\t', na_rep='NaN', header=True,
                                float_format='%g')

    def write_args_log(s, sim_args_dict):
        filepath = Path(QmEnergyLogger.log_folder() + '/' + s.get_logfile_name(logtype_="Args") + ".json")
        with open(filepath, 'w') as f:
            json.dump(sim_args_dict, f)
...
def test_01(seed=0, subbands=1, isd=5000.0, sim_radius=2500.0, nues=1, until=100.0, author='Kishan Sthankiya',
    ...
    for centre in sim_hexgrid_centres[:]:
        x, y = centre
        z = 25.0
        sim.make_cell(interval=1.0, xyz=[x, y, z], n_subbands=subbands)
    ...
    energy_logger = QmEnergyLogger(sim=sim, seed=seed, energy_model=em, until=until, sim_args=sim_args_dict)
    cell_logger = QmCellLogger(sim=sim, seed=seed, energy_model=em, until=until, sim_args=sim_args_dict)
    sim.add_loggers([energy_logger, cell_logger])  # std_out & dataframe
