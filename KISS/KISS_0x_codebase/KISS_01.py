...
class MyLogger(Logger):
    ...
    def __init__(self, sim,func=None,header='',f=stdout,logging_interval=1.0,np_array_to_str=np_array_to_str):
        self.dataframe = None
        super(MyLogger, self).__init__(sim,func=None,header='',f=stdout,logging_interval=1.0,np_array_to_str=np_array_to_str)
    ...
    def get_data(self):
        ...
        columns = ["time","serving_cell_id","UE_id","X","Y","throughput(Mb/s)"]
        for cell in self.sim.cells:
                for attached_ue_id in cell.attached:
                    UE = self.sim.UEs[attached_ue_id]
                    tm = self.sim.env.now                        # current time
                    sc = UE.serving_cell.i                       # current UE serving_cell ID
                    id = UE.i                                    # current UE ID
                    xy = UE.get_xyz()[:2]                        # current UE position
                    tp = cell.get_UE_throughput(attached_ue_id)  # current UE throughput

                    # Write above to `data` list
                    data.append([tm, sc, id, xy[0], xy[1], tp])
        return columns, data
    ...
    def add_to_dataframe(self, col_labels, new_data, ignore_index):
        if self.dataframe is None:
            ...
            self.dataframe = pd.concat([self.dataframe,new_data_df], verify_integrity=True, ignore_index=ignore_index)

    def run_routine(self, ignore_index=False):
        col_labels, new_data = self.get_data()
        self.add_to_dataframe(col_labels=col_labels, new_data=new_data, ignore_index=ignore_index)
