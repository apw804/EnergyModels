...
def create_logfile_name(seed):
    """Create a filename for the log file based on the script name and current timestamp."""
    script_name = Path(__file__).stem
    timestamp = datetime.now()
    timestamp_iso = timestamp.isoformat(timespec='seconds')
    seed_str = str(seed)
    log_filename = '_'.join([script_name, 'logfile', seed_str, timestamp_iso]) + '.tsv'
    return log_filename
...
class MyLogger(Logger):
    ...
    def __init__(self, *args,cell_energy_models=None, log_filename=None, **kwargs):
        self.dataframe = None
    ...
    def finalize(self):
        ...
        df1.to_csv(self.log_filename, sep="\t", index=False, mode='w')
...
def main(config_file):
    ...
    SetCellSleep_bedtime_stories = config["scenario_SetCellSleep_params"]["SetCellSleep_bedtime_stories"]
    ...
    logfile_name = create_logfile_name(seed=seed)
    ...
    custom_logger = MyLogger(sim, logging_interval=base_interval, cell_energy_models=cell_energy_models_dict, log_filename=create_logfile_name(seed=seed))
...
if __name__ == '__main__':
    ...
    # Call main with the current seed value
    dataframe = main(config_file=args.config_file)