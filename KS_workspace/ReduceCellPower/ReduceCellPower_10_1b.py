...
class CellEnergyModel:
    ...
    def __init__(s,
        ...
        s.cell_power_static_kW = s.params.power_static_watts / 1000  # baseline energy use in kilowatts

        if cell.sim.env.now == 0.:
            s.cell_power_kW = s.cell_power_static_kW  # Instantaneous independent power in kilowatts
        else:
    ...
    def update_cell_power(s):
        ...
        # Get the scaling factor for the PA efficiency
        eta_pa_max = s.params.eta_pa
        pa_macro_gamma = s.params.gamma_pa
        x = (s.cell.get_power_dBm() / s.params.p_max_db)
        eta_pa_scaled = eta_pa_max * (1 - pa_macro_gamma * np.log2(x))

        # Get the value of the power consumptions (numerator) of a single sector/antenna
        power_consumption = s.params.power_baseband_watts + s.params.power_rf_watts + (
                    (s.params.p_max_db / eta_pa_scaled * (1 - s.params.loss_feed_db)))
        ...
        power_out_watts = power_consumption / power_losses

        # Calculate the linear approximation of the cell power model and covert to kW
        s.cell_power_kW = s.cell_antennas * (s.params.power_static_watts + (s.cell.get_power_dBm() / s.params.p_max_db) * power_out_watts) / 1000
...
def test_01(seed=0, subbands=1, isd=5000.0, sim_radius=2500.0, nues=1, until=100.0, power_dBm=43.0, author='Kishan Sthankiya',
...
if __name__ == '__main__':  # a simple self-test
    ...
    test_01(seed=args.seed, subbands=args.subbands, isd=args.isd, sim_radius=args.sim_radius, power_dBm=args.power_dBm, nues=args.nues,
