...
class Cellv2(Cell):
    ...
    def check_sleep_mode(self):
        """
        Checks the self.sleep_mode. If set to a number between 1-4, changes behaviour of Cell and associated energy consumption.
        Default: self.sleep_mode = 0 (NO SLEEP MODE) - Cell is ACTIVE
        """
        # If the cell is in REAL sleep state (1-5):
        if self.sleep_mode in self._SLEEP_MODES[1:]:  
            ...
            # ALL attached UE's should be detached and handover to available cells
            for i in self.attached:
                pass
                # FIXME - Should we only do handovers for the orphaned UEs? Would have to have to change AMFv1 logic to achieve this.
...
class AMFv1(MME):
    ...
    def attach_ues_to_best_rsrp(self, ues):
        ...
        for ue_i in ues:
            ...
            celli=self.sim.get_best_rsrp(ue_i)
...
class ChangeCellPower(Scenario):
    ...
    def loop(self):
        self.sim.cells[2].set_sleep_mode(mode=1)
        while True:
