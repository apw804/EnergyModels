  ...
  def loop(s):
    ...
    while True:
      ...
      for cell in s.sim.cells:
        ...
        # Write to stdout
        s.f.write(
          f"{tm:10.2f}\t{cell_i:2}\t{n_UEs:2}\t{tp:10.2f}\t{ec:10.2f}\t{ee:10.2f}\n"
        )  
...
def em001(ncells=1,nues=1,until=10):
  ...
  sim.add_scenario(QmScenario(sim,verbosity=1))
  sim.run(until=until)
  print(f'reference_signal_power={em.rsp_dBm(cell)}dBm')
